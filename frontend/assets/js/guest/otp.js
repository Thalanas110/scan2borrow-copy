class GuestOtpController {
    constructor() { this.form = document.getElementById("guest-otp-form") || document.getElementById("profile-otp-form"); this.resend = document.getElementById("resend-form"); this.bindEvents(); }
    submit(event) { event.preventDefault(); fetch("/scan2borrow/api/auth/guest/otp", { method: "POST", body: new FormData(this.form) }).then((response) => response.json()).then((response) => { if (!response.ok) throw new Error(response.errors?.[0] || "Invalid or expired OTP code."); window.location.href = "/scan2borrow/guest/dashboard"; }).catch((error) => this.message("form-error", error.message)); }
    resendOtp(event) { event.preventDefault(); fetch("/scan2borrow/api/auth/guest/otp/resend", { method: "POST", body: new FormData(this.resend) }).then((response) => response.json()).then((response) => this.message(response.ok ? "form-success" : "form-error", response.message || response.errors?.[0] || "Please wait before requesting another code.")); }
