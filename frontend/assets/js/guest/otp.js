class GuestOtpController {
  constructor() {
    this.form =
      document.getElementById("guest-otp-form") ||
      document.getElementById("profile-otp-form");
  ...
  submit(event) {
    ...
    fetch("/scan2borrow/api/auth/guest/otp", { method: "POST", body })
      ...
      .then((response) => {
        ...
        window.location.href = "/scan2borrow/guest/dashboard";
      })
      .catch((error) => this.message("form-error", error.message));
  }
  resendOtp(event) {
    event.preventDefault();
    const body = new FormData(this.resend);
    body.append("csrf", this.csrf);
    fetch("/scan2borrow/api/auth/guest/otp/resend", { method: "POST", body })
      .then((response) => response.json())
      .then((response) =>
        this.message(
  ...
  bindEvents() {
    ...
    this.resend?.addEventListener("submit", (event) => this.resendOtp(event));
  }
}
