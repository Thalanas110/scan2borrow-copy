class OtpPageController {
  constructor() {
    ...
    document
      ...
      ?.addEventListener("submit", (event) => this.resend(event));
  }
  ...
  submit(event) {
    ...
    this.request(event.currentTarget, "/scan2borrow/api/auth/otp")
      .then((response) => {
        if (!response.ok)
          throw new Error(
            response.errors?.[0] || "Invalid or expired OTP code.",
          );
        location.href = response.data.redirect || "/scan2borrow/login";
      })
      .catch((error) => this.message("form-error", error.message));
  }
  resend(event) {
    event.preventDefault();
    this.request(event.currentTarget, "/scan2borrow/api/auth/otp/resend").then(
      (response) =>
        this.message(
