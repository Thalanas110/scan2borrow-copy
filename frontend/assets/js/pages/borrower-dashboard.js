class BorrowerDashboardController {
    ...
    request(path, options = {}) {
        const requestOptions = Object.assign({ headers: { "X-Requested-With": "fetch" } }, options);
    ...
    load() {
        this.request(this.api).then((response) => {
            if (!response.ok) throw new Error(response.message || "Unable to load your dashboard.");
    ...
    render(data) {
        ...
        this.$("borrower-meta").textContent = [user.barcode, user.course ? ` · ${user.course}` : "", user.year_level ? ` · Year ${user.year_level}` : ""].join("");
        ...
        this.$("on-time-rate").textContent = `${Number(stats.on_time_rate ?? 100)}%`;
        this.renderCapacity(active, limit);
        this.renderOverdue(overdue);
        this.renderDueSoon(data.due_soon || []);
        this.renderRecommendations(data.recommended || [], data.favorite_category || "");
    ...
    renderCapacity(active, limit) {
        ...
        ring.style.setProperty("--ring-color", active >= limit ? "var(--danger)" : "var(--primary)");
        this.$("capacity-value").textContent = `${active}/${limit}`;
        this.$("capacity-remaining").textContent = `${Math.max(0, limit - active)} slot(s)`;
    ...
    renderDueSoon(items) {
        ...
        if (!items.length) {
            ...
            host.textContent = "Nothing due in the next 3 days. You're all caught up.";
            return;
        }
    ...
    renderRecommendations(items, favoriteCategory) {
        ...
        reason.innerHTML = favoriteCategory ? `Because you enjoy <strong>${this.escapeHtml(favoriteCategory)}</strong>` : (items.length ? "Based on your recent searches and interests" : "Popular available titles");
        host.replaceChildren();
        if (!items.length) {
            host.className = "text-muted small";
            host.textContent = "No available books to recommend right now. Try searching for something you like!";
            return;
        }
        items.forEach((book) => {
            const link = document.createElement("a");
            link.href = "/scan2borrow/student/search?search=" + encodeURIComponent(book.title || "");
            link.className = "rec";
            link.style.cssText = "text-decoration:none;color:inherit;";
            link.innerHTML = `<div class="rec-cover">${this.escapeHtml(String(book.title || "").slice(0, 1).toUpperCase())}</div><div class="flex-grow-1"><div class="rec-t">${this.escapeHtml(book.title)}</div><div class="rec-m">${this.escapeHtml(book.author)} · ${this.escapeHtml(book.category)}</div></div><span class="badge bg-light text-muted border">&#128205; Flr ${this.escapeHtml(book.floor_no)}</span>`;
    ...
    renderLoans(loans) {
        ...
        if (!loans.length) {
            body.innerHTML = '<tr><td colspan="5" class="text-center text-muted py-4">You have no active borrowed. Tap <strong>Borrow a Book</strong> to get started.</td></tr>';
            return;
        }
        loans.forEach((loan) => {
    ...
    formatDate(value) {
        ...
        return Number.isNaN(date.valueOf()) ? String(value) : date.toLocaleDateString("en-US", { month: "short", day: "2-digit", year: "numeric" });
    }

    badge(status) {
        const type = { Borrowed: "primary", Overdue: "danger", Pending: "warning text-dark", Returned: "success" }[status] || "secondary";
        return `<span class="badge bg-${type}">${this.escapeHtml(status)}</span>`;
    }

    renderBarcode(barcode) {
        if (barcode && window.JsBarcode) {
            window.JsBarcode("#lib-barcode", barcode, { format: "CODE128", displayValue: false, margin: 0, height: 60, width: 2, lineColor: "#0f172a" });
        }
    }
    ...
    showBorrowSuccess(data) {
        this.$("successMessage").textContent = `Successfully borrowed ${Number(data.book_count || 0)} book(s)!`;
        this.$("successTxnCode").textContent = data.transaction_code || "";
        this.$("successBookCount").textContent = String(data.book_count || 0);
        this.$("successReceiptLink").href = "/scan2borrow/receipt?code=" + encodeURIComponent(data.transaction_code || "");
    ...
    bindEvents() {
        this.borrowForm?.addEventListener("submit", (event) => {
            ...
            this.submitForm(this.borrowForm, "borrow", "book_barcode", (data) => this.showBorrowSuccess(data));
        });
        this.returnForm?.addEventListener("submit", (event) => {
            event.preventDefault();
            this.submitForm(this.returnForm, "return_unified", "return_input", (data) => {
                this.$("return-message").hidden = false;
                this.$("return-message").textContent = data.message || "Book returned successfully.";
            });
        });
        window.addEventListener("load", () => this.renderBarcode(this.$("borrower-barcode").textContent));
    }
}

window.addEventListener("DOMContentLoaded", () => new BorrowerDashboardController());
