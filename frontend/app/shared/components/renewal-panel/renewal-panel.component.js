export class RenewalPanelComponent {
  ...
  render(loans, renewals) {
    ...
    const rows = activeLoans.map((loan) => {
      ...
      return `<div class="renewal-panel__row" data-loan-id="${this.escape(loanId)}"><div><strong>${this.escape(loan.title)}</strong><span>Due ${this.escape(loan.due_date || '—')}</span></div><div>${state}</div></div>`;
    }).join('');
    this.root.innerHTML = `<section class="renewal-panel"><div class="renewal-panel__header"><span class="renewal-panel__eyebrow">Borrower request</span><h2>Renewals</h2><p>Requests add one standard period and require librarian approval.</p></div>${rows}</section>`;
  }
