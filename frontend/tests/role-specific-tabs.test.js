...
test('teacher catalog and history have independent templates and navigation ownership', () => {
  ...
  assert.match(catalog, /teacher-library-surfaces\.css/);
  assert.match(catalog, /teacher-search\.css/);
  assert.doesNotMatch(catalog, /student-(?:search|library-surfaces)\.css/);
