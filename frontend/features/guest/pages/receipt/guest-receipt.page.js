  ...
  load() {
    ...
    if (this.service) {
      this.service.load(id)
        ...
        .catch((error) => {
          this.host.innerHTML = `<div class="alert alert-danger">${this.escapeHtml(error.message || "Receipt not found.")}</div>`;
        });
      return;
    }
...
if (typeof window !== "undefined") {
  window.addEventListener("DOMContentLoaded", () => new GuestReceiptPage());
}
