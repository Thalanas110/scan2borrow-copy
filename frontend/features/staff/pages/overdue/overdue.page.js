export class OverduePage {
  ...
  render(data) {
    ...
    body.innerHTML = rows.length
      ? rows.map((row) => `<tr class="row-overdue"><td>${this.escape(row.borrower)}</td><td>${this.escape(row.title)}</td><td>${Number(row.quantity || 1)}</td><td>${this.escape(row.due_date)}</td><td>${row.days_late || 0} day(s)</td><td>${Number(row.fine_amount || 0).toFixed(2)}</td><td><a href="/scan2borrow/staff/notify?id=${row.user_id}" class="btn btn-warning btn-sm">Email Reminder</a></td></tr>`).join('')
