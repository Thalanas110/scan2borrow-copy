...
final class MediaAssetContractTest extends TestCase
{
    ...
    public function testImageConsumersUseTheSharedMediaResolver(string $script, string $page): void
    {
        ...
        $scriptPath = $root . DIRECTORY_SEPARATOR . 'frontend' . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $script);
        $pagePath = $root . DIRECTORY_SEPARATOR . 'frontend' . DIRECTORY_SEPARATOR . 'pages' . DIRECTORY_SEPARATOR . $page;
    ...
    {
        return [
            ['pages/student-search.js', 'student-search.html'],
            ['pages/inventory.js', 'staff-books.html'],
            ['pages/staff.js', 'staff-dashboard.html'],
            ['pages/staff.js', 'staff-borrower.html'],
            ['pages/staff.js', 'staff-guest-requests.html'],
            ['guest/browse.js', 'guest-browse-books.html'],
            ['guest/borrowed.js', 'guest-borrowed-books.html'],
        ];
    }

    public function testGuestRequestInlinePhotoHandlersUseTheSharedResolver(): void
    {
        $path = dirname(__DIR__, 3) . DIRECTORY_SEPARATOR . 'frontend' . DIRECTORY_SEPARATOR . 'pages' . DIRECTORY_SEPARATOR . 'staff-guest-requests.html';
        $source = (string) file_get_contents($path);
